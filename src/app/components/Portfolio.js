...
export default function Portfolio({ theme }) {
  const projects = [
    ...
    { title: "Brand Campaign", img: "/51.jpeg" },
  ...
  return (
    <section className={`py-5 ${theme === "dark" ? "dark-bg" : "light-bg"}`}>
      ...
      <style jsx>{`
        .gradient-text { background: linear-gradient(135deg, #6a11cb 0%, #2575fc 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        .portfolio-card { border-radius: 12px; transition: transform 0.3s, box-shadow 0.3s, background 0.3s; overflow: hidden; }
        .portfolio-card:hover { transform: translateY(-10px); box-shadow: 0 12px 24px rgba(0,0,0,0.15); }
